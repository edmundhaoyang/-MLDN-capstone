...
launch = Tk()
launch.title("Desmond's Capstone project") 
launch.iconbitmap(default='Robotic_pet.ico')
...
b=15
(x, y) = (a, b)
board = Canvas(launch, width=x*b_border, height=y*b_border)
player = (0, y-1)
score = 1
restart = False
walk_reward = -0.04
...
agent = board.create_oval(player[0]*Width+Width*2/10, player[1]*Width+Width*2/10,
                            player[0]*Width+Width*8/10, player[1]*Width+Width*8/10, fill="blue", width=1, tag="katabullet")    
...
def render_grid():
    ...
    for (i, j, colored, w) in winning:
        board.create_rectangle(i*Width, j*Width, (i+1)*Width, (j+1)*Width,fill=colored,  width=1)
    for (i, j) in walls:
...
def try_move(dx, dy):
    ...
    if (new_x >= 0) and (new_x < x) and (new_y >= 0) and (new_y < y) and not ((new_x, new_y) in walls):
        ...
        player = (new_x, new_y)
    for (x_axis_goal, y_axis_goal, color, earnin_point) in winning:
        if new_x == x_axis_goal and new_y == y_axis_goal:
            score -= walk_reward
            score += earnin_point
            if score > 0 and score <1.90:
...
#binding the movement funtions to keyboard for manual tries
launch.bind("<Up>", call_up)
launch.bind("<Down>", call_down)
launch.bind("<Right>", call_right)
launch.bind("<Left>", call_left)
